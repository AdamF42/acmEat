...
//distance.key("AIzaSyDWPXlrmM_fkqGpHVtD72RIvhOPl_HGO5A")
...
app.get('/', (req, res) => { log("key "+MAPS_API_KEY)
    ...
    distance.matrix([from], [to], (err, distances) => {
        log('[Response] Status: ' + distances.status)
        if (err) {
    ...
    return res.status(200).json(response(distances.status, ""))
